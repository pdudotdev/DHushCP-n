...
MAX_MESSAGE_LENGTH = 15    # Maximum message length in characters
...
def get_wireless_interface():
    ...
    try:
        ...
        if len(interfaces) > 1:
            print(Style.BRIGHT + "[INFO] " + Style.RESET_ALL + "Multiple wireless interfaces detected. Please choose one:")
...
def get_limited_input(prompt_message, max_length):
    ...
    try:
        ...
        print(Style.BRIGHT + "\n[INPUT] " + Style.RESET_ALL + "Input cancelled by user.")
...
def encode_stego_message(cover_text, plaintext):
    """Embed plaintext into cover_text using zero-width characters mapping 2 bits per character."""
    # Convert plaintext to binary
    binary_message = ''.join(format(ord(c), '08b') for c in plaintext)
    
    # Pad binary_message to make its length a multiple of 2
    ...
    zero_width_message = ''.join(
        zero_width_chars[binary_message[i:i+2]] for i in range(0, len(binary_message), 2)
    )
    
    # Embed zero-width characters into the cover text
    stego_text = ''
    idx = 0
    for char in cover_text:
        stego_text += char
        if idx < len(zero_width_message):
            stego_text += zero_width_message[idx]
            idx += 1
...
def decode_stego_message(stego_text):
    """Extract plaintext from stego_text by decoding zero-width characters mapping 2 bits per character."""
    # Extract zero-width characters
    zero_width_chars_sequence = ''.join(
        c for c in stego_text if c in ['\u200B', '\u200C', '\u200D', '\u2060']
    )
    ...
    binary_message = ''.join(char_to_bits[c] for c in zero_width_chars_sequence)
...
def initiate_key_exchange(iface, session_id, dhushcp_id, private_key):
    ...
    if options is None:
        print(Style.BRIGHT + "[ERROR] " + Style.RESET_ALL + "Failed to embed public key into DHCP option.")
        return
    packet = create_dhcp_discover(session_id, dhushcp_id, options)
    send_dhcp_discover(packet, iface)
    print(Style.BRIGHT + "[INFO] " + Style.RESET_ALL + "Initiated key exchange by sending public key.")
...
def handle_received_dhcp(packet):
    ...
    if DHCP in packet and packet[DHCP].options:
        ...
        if (DHCP_OPTION_ID in option_dict and option_dict[DHCP_OPTION_ID] == DHUSHCP_ID and
            ...
            if not data_options:
                print(Style.BRIGHT + "[WARNING] " + Style.RESET_ALL + "No data embedded in DHCP Discover. Ignoring packet.")
            ...
            try:
                ...
                if session_id not in shared_key_holder or shared_key_holder[session_id]['key'] is None:
                    print(Style.BRIGHT + "[WARNING] " + Style.RESET_ALL + "Received encrypted message but shared key is not established.")
                    return
                plaintext = decrypt_message(shared_key_holder[session_id]['key'], assembled_data)
                if plaintext:
                    # Display the stego message
                    print(Style.BRIGHT + Fore.YELLOW + "\n[MESSAGE IN TRANSIT WAS] " + Style.RESET_ALL + f"{plaintext}")
                    
                    # Extract the plaintext message from the stego message
                    extracted_message = decode_stego_message(plaintext)
                    
                    # Display the extracted plaintext message
                    print(Style.BRIGHT + Fore.GREEN + "[REAL MESSAGE RECEIVED] " + Style.RESET_ALL + f"{extracted_message}\n")
                    
                    # Prompt user to reply
                    user_reply = get_limited_input("-> Enter your reply (max 15 characters, or press Ctrl+C to exit and cleanup):\n", MAX_MESSAGE_LENGTH)
                    if user_reply is None:
                        print(Style.BRIGHT + "[INFO] " + Style.RESET_ALL + "No reply entered, time to cleanup.")
                        return
                    if user_reply:
                        # Define a cover text - USE NO MORE THAN 5 CHARACTERS!!!
                        cover_text = "Nothing."
                        
                        # Embed the plaintext into the cover text
                        stego_reply = encode_stego_message(cover_text, user_reply)
                        if stego_reply is None:
                            print(Style.BRIGHT + "[ERROR] " + Style.RESET_ALL + "Failed to generate stego message.")
                            return
                        
                        # Display the stego reply to the user
                        print(Style.BRIGHT + Fore.YELLOW + "\n[MESSAGE IN TRANSIT IS] " + Style.RESET_ALL + f"{stego_reply}\n")
                        
                        encrypted_reply = encrypt_message(shared_key_holder[session_id]['key'], stego_reply)
            ...
            # Packet does not match DHushCP criteria or session ID. Ignoring.
            pass
...
def cleanup_process():
    ...
    try:
        private_key = None
        public_key = None
        shared_key_holder[session_id]['key'] = None
        gc.collect()
...
def main():
    ...
    print(Style.BRIGHT + "[INFO] " + Style.RESET_ALL + "Initiator will listen for DHCP Discover packets.")

    # Initiator prompts to initiate communication
    choice = input(Style.BRIGHT + "[INPUT] " + Style.RESET_ALL + "Do you want to initiate communication? (y/n): ").strip().lower()
    if choice == 'y':
        initiate_key_exchange(iface, session_id, DHUSHCP_ID, private_key)

    # Wait until shared key is established
    try:
        while session_id not in shared_key_holder and not stop_event.is_set():
            time.sleep(1)
    ...
    # Prompt for message
    user_message = get_limited_input("\n-> Enter your message to send (max 15 characters, or press Ctrl+C to exit and cleanup):\n", MAX_MESSAGE_LENGTH)
    ...
    if user_message:
        ...
        # Embed the plaintext into the cover text
        stego_message = encode_stego_message(cover_text, user_message)
        if stego_message is None:
            print(Style.BRIGHT + "[ERROR] " + Style.RESET_ALL + "Failed to generate stego message.")
            cleanup_process()
            sys.exit(1)
        
        # Display the stego message to the user
        print(Style.BRIGHT + Fore.YELLOW + "\n[MESSAGE IN TRANSIT IS] " + Style.RESET_ALL + f"{stego_message}\n")
        
        # Encrypt the stego message
        encrypted_message = encrypt_message(shared_key_holder[session_id]['key'], stego_message)
        
        packet_options = embed_data_into_dhcp_option(encrypted_message)
        if packet_options is None:
            print(Style.BRIGHT + "[ERROR] " + Style.RESET_ALL + "Failed to embed encrypted message into DHCP option.")
            cleanup_process()
            sys.exit(1)
        message_packet = create_dhcp_discover(session_id, DHUSHCP_ID, packet_options)
        send_dhcp_discover(message_packet, iface)
        print(Style.BRIGHT + "[INFO] " + Style.RESET_ALL + "Sent encrypted message.")
